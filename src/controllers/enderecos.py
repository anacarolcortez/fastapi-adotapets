...
async def create_address(address_collection, address, email):
    ...
    address = await create_client_address(address_collection, address_data, email)
    if address is not None:
        return address    
    raise HTTPException(status_code=400, detail="Erro na criação do endereço do adotante")


async def find_address(address_collection, email):
    address = await get_address_by_adopter_email(address_collection, email)
    if address:
        return address
    raise HTTPException(status_code=404, detail="Não há endereço cadastrado para o adotante")


async def update_address(address_collection, address, email):
    address_data = jsonable_encoder(address)
    updated_address = await update_address_by_email(address_collection, address_data, email)
    if updated_address:
        return await get_address_by_adopter_email(address_collection, email)
    raise HTTPException(status_code=400, detail="Erro ao atualizar endereço")
    

async def delete_address(address_collection, email):
    deleted_address = await delete_address_by_email(address_collection, email)
    if deleted_address:
        return {
            "status_code": 200,
